...
void MlmeEventHandlerImpl::OnConnect(
    const unique_ptr<const MlmeConnectEvent> event) {
...
void MlmeEventHandlerImpl::OnRoam(const unique_ptr<const MlmeRoamEvent> event) {
  if (event->GetStatusCode() == 0) {
    client_interface_->RefreshAssociateFreq();
    client_interface_->bssid_ = event->GetBSSID();
  }
...
void MlmeEventHandlerImpl::OnAssociate(
    const unique_ptr<const MlmeAssociateEvent> event) {
  if (event->GetStatusCode() == 0) {
    client_interface_->RefreshAssociateFreq();
    client_interface_->bssid_ = event->GetBSSID();
  }
