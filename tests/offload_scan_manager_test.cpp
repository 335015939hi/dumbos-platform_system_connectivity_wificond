...
class OffloadScanManagerTest: public ::testing::Test {
  protected:
    virtual void SetUp() {
      mock_offload_service_utils_.reset(new NiceMock<MockOffloadServiceUtils>());
      ...
      mock_offload_ = new NiceMock<MockOffload>();      
      ScanResult scanResult;
      vector<uint8_t> ssid(kSsid, kSsid + sizeof(kSsid));
      scanResult.tsf = 0;
      ...
      dummy_scan_results_.push_back(scanResult);
    }
    ...
    sp<NiceMock<MockOffload>> mock_offload_;
...
TEST_F(OffloadScanManagerTest, ServiceUtilsNotAvailableTest) {
  dut_ = unique_ptr<OffloadScanManager>(new OffloadScanManager(
            nullptr, nullptr));
...
TEST_F(OffloadScanManagerTest, ServiceNotAvailableTest) {
  ...
  dut_ = unique_ptr<OffloadScanManager>(new OffloadScanManager(
    mock_offload_service_utils_.get(), nullptr));
  EXPECT_EQ(false, dut_->isServiceAvailable());
}
