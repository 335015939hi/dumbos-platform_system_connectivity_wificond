...
status_t ParcelableRttParams::writeToParcel(Parcel* parcel) const {
  ...
  for (auto& params : params_list) {
    ...
    if (status != OK) return status;
    status = parcel->writeByte(params.secure ? 1 : 0);
    if (status != OK) return status;
    ...
    if (status != OK) return status;
    status = parcel->writeInt32(params.lci_request ? 1 : 0);
    if (status != OK) return status;
    status = parcel->writeInt32(params.lcr_request ? 1: 0);
    if (status != OK) return status;
...
status_t ParcelableRttParams::readFromParcel(const Parcel* parcel) {
  ...
  for (int i = 0; i < num; i++) {
    ...
    params.secure = (parcel->readByte() != 0);
    ...
    params.lci_request = (parcel->readInt32() == 1);
    params.lcr_request = (parcel->readInt32() == 1);
