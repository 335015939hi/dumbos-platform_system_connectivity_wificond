...
void NetlinkManager::BroadcastHandler(unique_ptr<const NL80211Packet> packet) {
  ...
  // Driver which supports SME uses NL80211_CMD_AUTHENTICATE and
  // NL80211_CMD_ASSOCIATE, otherwise it uses NL80211_CMD_CONNECT
  // to performe both authentication and association processses.
  ...
  if (command == NL80211_CMD_CONNECT ||
    ...
    MlmeEventHandler(std::move(packet));
    return;
  }
}

void NetlinkManager::MlmeEventHandler(unique_ptr<const NL80211Packet> packet) {
  uint32_t if_index;
  ...
  unique_ptr<MlmeEvent> mlme_event = MlmeEvent::InitFromPacket(packet.get());
  if (mlme_event == nullptr) {
    LOG(ERROR) << "Failed to init MlmeEvent from a NL80211 Packet";
    return;
  }
  on_mlme_event_handler_[if_index](std::move(mlme_event));
}
