...
bool NetlinkUtils::GetPacketCounters(uint32_t interface_index,
                                     const vector<uint8_t>& mac_address,
                                     vector<int64_t>* out_counters) {
  ...
  uint32_t rx_good, rx_bad;
  if (!sta_info.GetAttributeValue(NL80211_STA_INFO_TX_PACKETS, &rx_good)) {
    LOG(ERROR) << "Failed to get NL80211_STA_INFO_TX_PACKETS";
    return false;
  }
  if (!sta_info.GetAttributeValue(NL80211_STA_INFO_TX_FAILED, &rx_bad)) {
    LOG(ERROR) << "Failed to get NL80211_STA_INFO_TX_FAILED";
    return false;
  }
  vector<int64_t> counters;
  counters.push_back(static_cast<int64_t>(rx_good));
  counters.push_back(static_cast<int64_t>(rx_bad));
  LOG(INFO) << "rx_good:" << rx_good;
  LOG(INFO) << "rx_bad:" << rx_bad;
