...
bool NetlinkUtils::GetWiphyIndex(uint32_t* out_wiphy_index) {
  ...
  for (auto& packet : response) {
    ...
    if (packet->GetCommand() != NL80211_CMD_NEW_WIPHY) {
      LOG(ERROR) << "Wrong command of response message for"
...
bool NetlinkUtils::GetWiphyCapabilities(
  ...
  if (!netlink_manager_->SendMessageAndGetSingleResponse(get_wiphy, &response))  {
  ...
  if (response->GetCommand() != NL80211_CMD_NEW_WIPHY) {
    LOG(ERROR) << "Wrong command of response message for a get wiphy request: "
               << static_cast<int>(response->GetCommand());
    return false;
  }
  uint8_t max_num_scan_ssids;
  if (!response->GetAttributeValue(NL80211_ATTR_MAX_NUM_SCAN_SSIDS,
                                   &max_num_scan_ssids)) {
  ...
  if (!response->GetAttributeValue(NL80211_ATTR_MAX_NUM_SCHED_SCAN_SSIDS,
                                   &max_num_sched_scan_ssids)) {
    LOG(ERROR) << "Failed to get the capacity of maximum number of scheduled scan ssids";
    return false;
  }

  uint8_t max_match_sets;
  if (!response->GetAttributeValue(NL80211_ATTR_MAX_MATCH_SETS,
                                   &max_match_sets)) {
  ...
  if (!response->GetAttribute(NL80211_ATTR_WIPHY_BANDS, &bands_attr)) {
  ...
  for (unsigned int band_index = 0; band_index < bands.size(); band_index++) {
    ...
    for (auto& freq : freqs) {
      ...
      if (freq.GetAttributeValue(NL80211_FREQUENCY_ATTR_DFS_STATE,
        ...
        // Since there is no guarantee for the order of band attribbutes,
        // we do some math here.
        if (frequency_value > 2400 && frequency_value < 2500) {
