...
bool NetlinkUtils::GetWiphyCapabilities(
  ...
  if (!GetBandInfo(response.get(), out_band_info) ||
      !GetScanCapabilities(response.get(), out_scan_capabilities)) {
    return false;
  }
  return true;
}

bool NetlinkUtils::GetScanCapabilities(const NL80211Packet* const packet,
                           ScanCapabilities* out_scan_capabilities) {
...
bool NetlinkUtils::GetBandInfo(const NL80211Packet* const packet,
                               BandInfo* out_band_info) {
