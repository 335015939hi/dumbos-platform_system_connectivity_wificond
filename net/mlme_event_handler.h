...
class MlmeEventHandler {
 public:
  ...
  virtual void OnConnect(
      const std::unique_ptr<const MlmeConnectEvent> event) = 0;
  virtual void OnRoam(const std::unique_ptr<const MlmeRoamEvent> event) = 0;
  virtual void OnAssociate(
      const std::unique_ptr<const MlmeAssociateEvent> event) = 0;
