...
typedef std::function<void(std::vector<ScanResult> scanResult)> OnScanResultReadyHandler;

class ScanResultHandler {
public:
  ScanResultHandler(OnScanResultReadyHandler);
  ~ScanResultHandler();
  void callback(const hidl_vec<ScanResult>&);
private:
  OnScanResultReadyHandler handler_;
};

class OffloadCallback : public IOffloadCallback {
public:
  explicit OffloadCallback(OnScanResultReadyHandler handler);
