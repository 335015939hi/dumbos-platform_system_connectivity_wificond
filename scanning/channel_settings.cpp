...
status_t ChannelSettings::writeToParcel(::android::Parcel* parcel) const {
  RETURN_IF_FAILED(parcel->writeInt32(frequency));
  RETURN_IF_FAILED(parcel->writeInt32(dwell_time_ms));
  RETURN_IF_FAILED(parcel->writeInt32(passive ? 1 : 0));
  return ::android::OK;
}

status_t ChannelSettings::readFromParcel(const ::android::Parcel* parcel) {
  RETURN_IF_FAILED(parcel->readInt32(&frequency));
  RETURN_IF_FAILED(parcel->readInt32(&dwell_time_ms));
  passive = (parcel->readInt32() != 0);
  return ::android::OK;
}
