...
Server::Server(unique_ptr<HalTool> hal_tool, unique_ptr<InterfaceTool> if_tool,
  ...
  hal_hidl_->registerEventCallback(this, [] (const WifiStatus& status) {
...
bool Server::SetupInterfaceForMode(int mode,
  ...
  hal_hidl_->getChip(0, [&hal_failure, this](const WifiStatus& status, const sp<IWifiChip>& chip) {
  ...
  hal_chip_hidl_->registerEventCallback(this, [&hal_failure] (const WifiStatus& status) {
    if (status.code != WifiStatusCode::SUCCESS) {
  ...
  hal_chip_hidl_->requestChipDebugInfo([&hal_failure] (const WifiStatus& status, const IWifiChip::ChipDebugInfo& debug) {
    if (status.code != WifiStatusCode::SUCCESS) {
      LOG(ERROR) << "Failed to get chip info!";
      hal_failure = true;
      return;
    }
    LOG(INFO) << "ChipInfo: " << debug.driverDescription.c_str() << "," << debug.firmwareDescription.c_str();
  });
  if (hal_failure) {
    return false;
  }
