...
Server::Server(unique_ptr<HalTool> hal_tool,
  ...
  hal_->registerEventCallback(this);
}
...
android::hardware::Return<void> Server::onStart() {
  LOG(ERROR) << "onStart";
  hal_->getChip([this](const sp<android::hardware::wifi::V1_0::IWifiChip>& chip){
...
android::hardware::Return<void> Server::onStartFailure(const android::hardware::wifi::V1_0::FailureReason& reason) {
...
android::hardware::Return<void> Server::onFailure(const android::hardware::wifi::V1_0::FailureReason& reason) {
  return Void();
}
android::hardware::Return<void> Server::onChipReconfigured(uint32_t mode_id) {
  return Void();
}
android::hardware::Return<void> Server::onChipReconfigureFailure(uint32_t mode_id, const android::hardware::wifi::V1_0::FailureReason& reason) {
  return Void();
}
android::hardware::Return<void> Server::onChipInfoAvailable(const android::hardware::wifi::V1_0::IWifiChipEventCallback::ChipInfo& info) {
  LOG(ERROR) << "Chip info: driver=" << info.driver_version.c_str() << ", firmware=" << info.firmware_version.c_str();
  return Void();
}
