...
Server::onDebugRingBufferConnectivityEventEntriesAvailable(
    const WifiDebugRingBufferStatus& status,
    const hidl_vec<WifiDebugRingEntryConnectivityEvent>& entries) {
  return Void();
}
android::hardware::Return<void>
Server::onDebugRingBufferPowerEventEntriesAvailable(
    const WifiDebugRingBufferStatus& status,
    const hidl_vec<WifiDebugRingEntryPowerEvent>& entries) {
  return Void();
}
android::hardware::Return<void>
Server::onDebugRingBufferWakelockEventEntriesAvailable(
    const WifiDebugRingBufferStatus& status,
    const hidl_vec<WifiDebugRingEntryWakelockEvent>& entries) {
  return Void();
}
android::hardware::Return<void>
Server::onDebugRingBufferVendorDataEntriesAvailable(
    const WifiDebugRingBufferStatus& status,
...
bool Server::SetupInterfaceForMode(int mode,
  ...
  hal_chip_hidl_->registerEventCallback(
      this, [&hal_failure](const WifiStatus& status) {
        if (status.code != WifiStatusCode::SUCCESS) {
