...
using android::hardware::wifi::V1_0::SupplicantStatus;
using android::hardware::wifi::V1_0::SupplicantStatusCode;
using android::hardware::wifi::V1_0::ISupplicant;
...
Status Server::createClientInterface(sp<IClientInterface>* created_interface) {
  ...
  ISupplicant::IfaceParams iface_params;
  iface_params.ifName = interface_name.c_str();
  ...
  supplicant_hidl_->createInterface(iface_params, callback);

  unique_ptr<ClientInterfaceImpl> client_interface(new ClientInterfaceImpl(
      interface_name, interface_index, interface_mac_addr,
      supplicant_manager_.get(), supplicant_iface_hidl, netlink_utils_,
      scan_utils_));
