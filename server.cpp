...
bool Server::SetupInterfaceForMode(int mode,
  ...
  if (!hal_chip_hidl_.get()) {
    return true;
  }
  hal_chip_hidl_->registerEventCallback(this, [] (const WifiStatus& status) {
    if (status.code != WifiStatusCode::SUCCESS) {
      LOG(ERROR) << "Failed to register callback! " << status.code;
    }
  });
  hal_chip_hidl_->requestChipDebugInfo([] (const WifiStatus& status, const IWifiChip::ChipDebugInfo& debug) {
    if (status.code != WifiStatusCode::SUCCESS) {
      LOG(ERROR) << "Failed to get chip info! " << status.code;
    }
